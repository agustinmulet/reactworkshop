...
también se repite bastante, no? ;D) Antes de hacer el componente, tenemos que
...
la semana calculando desde la fecha en formato UNIX que nos proveen en
`date_epoch`. Para hacerla es como hicimos antes cuando vimos
[Props y State](https://agustinmulet.github.io/reactworkshop/props-y-state/), pero debemos definir antes un arreglo con los
días de la semana, el cual va por fuera de la definición de nuestro componente.
...
**Pasar el id como key se considera malas prácticas** ya que si eliminamos o
movemos datos dentro del array, ese id puede ir cambiando. En nuestro caso la
`key` puede ser el día de la semana, sea en UNIX (`date_epoch`) o en string
(`date`), entonces pasamos eso como prop a lo que se itera, nos quedaría:
...
perfección! En la próxima parte vamos a ver cómo usar un 'Backend-as-a-Token'
(según dice en su página, yo lo veo más como un servicio de backend o cloud
functions, ya son muchas cosas y me pierdo 😵) para guardar nuestra API key y
subir nuestro proyecto a (por ejemplo) [Netlify](https://www.netlify.com) que es
un hosting que me gusta **MUCHISIMO**
